...
- **소스 우선순위**: 제공자료(모드 I) → 신규조사(모드 R). **모드 Q(vault 사전지식)는 `knowledge_vault`가 설정된 환경에서만 그 사이에 끼어든다** — 배포 기본값은 vault 미설정이며, 그 상태로 파이프라인 전 구간이 완결된다.
...
- 판정이 애매하면 `추정`으로 낮춘다 — 과대 확정 태깅은 후속 팩트체크(pipeline
  `references/factcheck.md` §A)
  단계의 신뢰를 깎는다.
...
### vault 승격 — vault가 끌어간다 (이 스킬은 쓰지 않는다)

**이 스킬은 vault에 아무것도 쓰지 않는다.** 승격은 claudian 쪽 `/ingest` **모드 H(하네스 수확)**
가 작업폴더를 읽어 **pull**하는 방식이다(claudian `CLAUDE.md` §5.2.1). 소유권이 vault에 있어야
그쪽 승인 게이트·스키마 계약이 온전히 적용되기 때문이다.

- 작업폴더 `research/`가 1차 저장소이자 스테이징이다 — vault 유무와 무관하게 그 자체로 완결된다.
- 조사 완료 시 §6 종료 요약 1줄 보고에 **"vault 적재 후보 N건 — claudian `/ingest`로 수확"**을
  포함한다. 이 스킬의 책임은 여기까지다.
- **vault의 어떤 파일도 쓰지 않는다** — `acquired/`·`wiki/log.md` 장부 기록도 예외 없다.
  이전 규약(이 스킬이 승인 후 직접 복사)은 폐지됐다: 건 로컬 슬러그가 건 간에 충돌해
  (`vault-사전지식`·`코드베이스분석` 등) flat 적재 시 덮어쓰기가 나고, 프론트매터·manifest
  스키마도 vault 계약(§4.8)과 달라 변환 없이는 규약 위반 파일이 된다. 두 문제 모두 수확하는
  쪽에서 푼다.
- **`{knowledge_vault}/`는 읽기 전용으로 취급한다.** 모드 Q로 읽고 인용할 뿐이다.
- 수확된 적재가 다음 건의 모드 Q 재사용으로 이어지는 지식 복리 고리다.
