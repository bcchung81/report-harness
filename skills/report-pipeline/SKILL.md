...
127.0.0.1 전용·표준 라이브러리·외부 전송 없음. 게이트 밖에서 사용자가 직접 열 때는 `/report-review [작업폴더|건명|all|닫기]`
커맨드가 같은 절차(serve → wait → 게이트② 처리)를 시작한다('26.9.25 사용자 지시).
...
3. **인도**: `final/r{NN}_{YYYYMMDD}_{제목}.hwpx`를 파일 첨부로 전송(SendUserFile류)한다. 미검증 사실(팩트체크
   ...
   **쪽수가 분량을 넘어도 승인된 초안을 줄이지 않는다.** 후처리 JSON의 `layout.est_pages`(R067)가 게이트⓪ 분량을
   넘으면 인도 보고에 1줄로 알리고, 줄일 곳(절·표 지정 / 붙임 이관 / 그대로 둠)은 선택지로 사용자가 고른다. 게이트②
   승인 뒤 스스로 문구를 합치거나 빼면 사용자가 보지 않은 판이 인도된다('26.9.25 하네스 실전 점검 — 승인 뒤 개요
   항목·※ 줄을 합쳐 1쪽에 맞췄다. 분량 맞추기 임의 압축은 '26.9.24 반려 사유이기도 하다). 물을 수 없는 실행이면
   그대로 인도하고 초과를 알린다.
