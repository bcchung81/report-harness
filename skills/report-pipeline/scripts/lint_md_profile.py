...
LOOKALIKE = {"\u2212": "-", "\u2024": "\u00b7", "\u2027": "\u00b7",
             "\u2010": "-", "\u2011": "-", "\ufe63": "-",
             # em·en 대시도 완성형 미수용이다 — 부제·동격 구분에 자주 섞여 들어오는데
             # 목록에 없어 검출을 빠져나갔다('26.9.8 lessons 기록 → '26.9.10 조치).
             # 구분자로 쓰였으면 하이픈이 아니라 **괄호**로 바꾼다(md-profile §1-3-2).
             "\u2014": "-", "\u2013": "-"}
...
# R026: 본문에서 붙임으로 설명을 미루는 인라인 유보 표기. 맺음의 "※ 세부내용은 붙임 참조"는
        ...
        if stripped.startswith("ㅇ") or stripped.startswith("○") or stripped.startswith("□"):
            ...
            if leading_spaces > depth_base:
                ...
                if leading_spaces < depth_base:
                    depth_base = leading_spaces
                bullet_run += 1
                # R045: 상위 계층 하나에 딸리는 하위 계층은 최대 1개 — 2개째부터 위반.
                # 초과분은 개수를 쳐내지 말고 성격이 가까운 항목끼리 통합 서술로 합친다.
                if bullet_run == 2:   # R045 '26.8.15 강화 — ㅇ 아래 대시 1개
