...
- 창 폭에 맞춘 쪽 배치 — 서랍을 열면 폭과 관계없이 쪽을 밀고, 모자라면 메모 폭 → 쪽 배율 순으로 줄인다
  ...
  (이전 wait 프로세스가 살아 기다리는 중이면 넘겨받지 않고, 어느 wait를 멈추면 되는지 알린다).
- **다른 출처 요청 차단** — Host가 로컬 주소가 아니거나 Origin이 같은 로컬 출처가 아니면 403. 사용자가 연 다른
  ...
  DNS 리바인딩으로 보고서를 읽어 가던 경로를 막는다. 같은 출처는 Origin과 Host 머리로 가려 포트 포워딩(SSH·
  VS Code)으로 연 화면도 그대로 쓴다.
...
- **외부 인용(R092)** — 본문 문구 + 시각화(인용 차트·재작도·원문 인용 블록·표) + `※ 자료:` 출처 줄. 출처 줄이 없는
  외부 인용 ※는 `source-line-missing` 경고. 분량을 이유로 시각화를 선택지에서 빼지 않는다. 외부 인용은 외부 주체
  (국가·국제기구·기관 약칭·자료명)와 근거어(연구·조사·통계·실적·% 등)가 함께 있을 때만 본다 — 운영 초안 14건 재점검에서
  내부 약호 `(T3)`·법령명·강조 괄호 「잠정」·내부 문서 참조·`※ 출처:` 줄까지 잡던 오탐 12건을 걸렀다(남은 2건은 실제
  출처 누락). 잇단 인용이 `;`로 함께 적은 출처 줄, `※ 주:` 단서 줄도 인정한다.
- 절 제목 어휘 검사는 풀 어휘에 붙은 번호·부제(`검토 결과 ① : …`)를 떼고 본다.
...
- 글자 없는 표는 표 수와 따로 센다(`blank_tables`) — 되읽기에 느는 빈 표(웹앱의 머리말 배너)는 잡음으로 넘기고,
  원본의 빈 표(서명란 등)가 빠지면 `count-mismatch:blank_tables`로 잡는다. 웹앱 compare도 `issues: []`가 정상이다.
...
### 배포·운영
- **규칙 시드 동기화(`sync_rules.py`)** — 종전에는 설치자 `rules.md`가 첫 실행 시드에 고정돼 새 규칙·정정이
  도달하지 않았다. 이제 시드에만 있는 규칙을 운영본 끝에 덧붙이고, 손대지 않은 옛 시드 줄은 새 줄로 바꾸며(시드에서
  폐지된 번호면 지운다), 대체·정정 표기·통합 마커·경위 로그(`rules-history.md`)도 맞춘다. 설치자가 고치거나 쌓은
  규칙은 번호가 겹쳐도 보존하고 보고만 한다. 자가진단에 '규칙 동기화' 항목.
- **시드 계보(`references/rules-seed.lineage.json`)** — 배포한 시드 줄의 지문. 격리 설치로 0.4.0 설치자의 갱신을
  재현하니, 계보 없이는 시드가 다시 쓴 옛 줄 7건을 '번호 충돌'로 오판해 "로컬 규칙을 옮기라"고 잘못 안내했고 폐지된
  R053이 운영본에 살아 있었다. 계보로 가려 14건 자동 갱신·1건 제거·로컬 규칙 보존까지 확인했다. 시드를 고치면
  `scripts/build_seed_lineage.py`로 갱신한다(테스트가 강제). 새 설치에서 경위 로그가 복사되지 않아 흡수된 번호 6건을
  '죽은 참조'로 경고하던 것, R9NN 로컬 규칙이 통합 증가분(+833)으로 세어지던 것도 고쳤다.
