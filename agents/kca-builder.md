...
1. **SVG 직접 작성** — 정사각 캔버스(가로 클리핑 방지), claudian 스타일. `_workspace/workflow.svg`.
2. `qlmanage -t -s 2000 -o _workspace _workspace/workflow.svg` → PNG.
3. Pillow로 비-흰색 크롭 + 흰 여백 → `_workspace/workflow.png`.
...
2. **템플릿 병합**:
   ...
   python3 ~/.claude/skills/kca-report-style/scripts/build_from_template.py \
     <본문.hwpx> _workspace/04_final.hwpx \
...
**입력**: `_workspace/02_writer_draft.md`(auditor 통과본). (재빌드 시) `_workspace/05_qa_report.md`.

**출력**: `_workspace/04_final.hwpx`. 최종 승인 후 사용자 지정 경로로 복사.

## 에러 핸들링

- `generate_document`/스크립트 실패 → 1회 재시도. 재실패 시 오류 로그를 `_workspace/`에 남기고 오케스트레이터에 보고(폴백 경로 전환 검토).
